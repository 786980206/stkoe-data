...
pub fn translate(
    ...
    if let Some(e) = expr {
        collect(e, dtype_of, &mut hint);
    }
    hint
}

fn collect(expr: &Expr, dtype_of: &dyn Fn(&str) -> Option<PDataType>, hint: &mut Hint) {
    match expr {
        Expr::BinaryExpr { left, op, right } if *op == Operator::And => {
            collect(left, dtype_of, hint);
            collect(right, dtype_of, hint);
        }
        Expr::BinaryExpr { left, op, right } => {
            if let Some((col, lit)) = col_literal(left, right) {
                apply(&col, op, lit, dtype_of, hint);
            }
        }
        _ => {}
    }
