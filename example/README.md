...
本目录包含两个 Python 脚本，展示 Splayed V1 列式存储引擎的完整生命周期。
...
| | demo_datafusion.py | demo_duckdb.py |
|---|---|---|
| 查询引擎 | DataFusion（原生集成） | DuckDB（CSV 交换） |
| 下推支持 | 投影 + 谓词下推到 Scanner | 无（CSV 全量导入） |
| SQL 执行 | 直接在 FIELD 文件上 | 在 DuckDB 内存表中 |
| 适合场景 | 低延迟点查 + 简单聚合 | 复杂分析 + 窗口函数 |
| Python 依赖 | 无 | duckdb |
